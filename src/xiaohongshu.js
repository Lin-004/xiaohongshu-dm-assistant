...
export async function launchBrowser() {
  ...
  return chromium.launchPersistentContext(config.paths.browserUserDataDir, {
...
export async function createInboxPage(context) {
...
export async function collectSelectorStats(page) {
  const stats = {};
...
export async function readUnreadConversations(page) {
  const result = await findConversationLocators(page);

  if (!result) {
    throw new Error('没有找到会话列表，请先手动进入小红书私信页，或调整选择器。');
  }

  logger.info(`使用会话选择器: ${result.selector}`);

  const conversations = [];
  const total = Math.min(await result.locator.count(), config.xiaohongshu.maxConversations);
...
export async function openConversation(page, index) {
  const result = await findConversationLocators(page);
  if (!result) {
...
export async function readConversationContext(page) {
...
async function readConversationTitle(page) {
  ...
  for (const selector of titleSelectors) {
    try {
      const text = normalizeText(await page.locator(selector).first().innerText({ timeout: 300 }).catch(() => ''));
      if (text) {
...
export async function sendReply(page, reply) {
  const inputResult = await findFirstVisible(page, config.selectors.messageInput, 800);
  if (!inputResult) {
  ...
  const sendResult = await findFirstVisible(page, config.selectors.sendButton, 800);
  if (!sendResult) {
