...
export async function ensureDeviceConnected(runtime) {
  ...
  if (!onlineDevices.length) {
    throw new AndroidChannelError(
      androidErrorCodes.deviceDisconnected,
      '没有检测到可用的 Android 设备，请先连接手机或启动模拟器。'
    );
  }
  ...
  if (runtime.deviceId && !onlineDevices.includes(runtime.deviceId)) {
    throw new AndroidChannelError(
      androidErrorCodes.deviceDisconnected,
      `未找到指定 Android 设备: ${runtime.deviceId}`
    );
  }
...
export async function dumpUiHierarchy(runtime) {
  ...
  if (!stdout.trim()) {
    throw new AndroidChannelError(
      androidErrorCodes.emptyUiDump,
      'Android UI dump 为空，无法读取当前页面。'
    );
  }
